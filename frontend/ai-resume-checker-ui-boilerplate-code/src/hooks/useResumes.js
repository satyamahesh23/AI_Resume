...
export function useResumesList() {
  return useQuery({
    ...
    queryFn: () => resumeApi.list().then((d) => d.resumes),
  });
}
...
export function useFullVersion(id, versionId) {
  return useQuery({
    ...
    queryFn: () => resumeApi.getVersion(id, versionId).then((d) => d.version),
...
export function useAnalysisForVersion(id, versionId) {
  return useQuery({
    ...
    queryFn: () => resumeApi.analysisForVersion(id, versionId).then((d) => d.analysis),
    enabled: !!id && !!versionId,
    retry: false,
  });
}
...
export function useAnalyses(id) {
  return useQuery({
    ...
    queryFn: () => resumeApi.analyses(id).then((d) => d.analyses),
    enabled: !!id,
  });
}
...
export function useUploadResume() {
  ...
  return useMutation({
    mutationFn: ({ file, title }) => resumeApi.upload(file, title),
    onSuccess: (data) => {
...
export function useAnalyzeResume(id) {
  ...
  const toast = useToast();
  return useMutation({
    mutationFn: (body) => resumeApi.analyze(id, body),
    onSuccess: (data) => {
...
export function useApplyRewrites(id) {
  ...
  const toast = useToast();
  return useMutation({
    mutationFn: (body) => resumeApi.rewrite(id, body),
    onSuccess: (data) => {
      qc.invalidateQueries({ queryKey: resumeKeys.detail(id) });
      qc.invalidateQueries({ queryKey: resumeKeys.list() });
      qc.invalidateQueries({ queryKey: dashboardKey });
...
export function useDiff(id, from, to, mode = "words") {
  return useQuery({
    ...
    queryFn: () => resumeApi.diff(id, from, to, mode),
